...
IS
  ...
  ls_irb_dept_admin   varchar2(1) := 'N';
...
BEGIN
  ...
  where t2.mbr_id in (select prncpl_id from krim_prncpl_t where lower(prncpl_nm) = lower(AV_USER_ID))
  ...
  inner join krim_role_mbr_t t2 on t1.role_id = t2.role_id
  where t2.mbr_id in (select prncpl_id from krim_prncpl_t where lower(prncpl_nm) = lower(AV_USER_ID))
  and t1.role_nm =  'Department IRB Administrator';
  ...
  if li_count > 0 then
    ...
    SELECT 
		...
		ls_irb_dept_admin AS IS_IRB_DEPT_ADMIN,
