...
public class QuestionnaireServiceImpl implements QuestionnaireService {
	...
	public QuestionnaireDto getQuestionnaireDetails(Integer moduleCode, String moduleItemId,Integer questionnaireAnswerHeader) throws Exception {
		...
		try {
			...
			ArrayList<HashMap<String, Object>> QuestionnaireAnswerDtoMap = new ArrayList<>();
			...
			ArrayList<HashMap<String, Object>> questionnaireQuestionsMap = dbEngine.executeProcedure(inParam,"get_mitkc_qnr_qstn", outParam);
			...
			ArrayList<HashMap<String, Object>> questionnaireConditionsMap = dbEngine.executeProcedure(inParam,"get_mitkc_qnr_qstn_condition", outParam);
			...
			ArrayList<HashMap<String, Object>> questionnaireOptionsMap = dbEngine.executeProcedure(inParam,"get_mitkc_qnr_qstn_option", outParam);
	...
	private ArrayList<HashMap<String, Object>> getQuestionnaireAnswer(Integer moduleCode, String moduleItemId) throws Exception {
		ArrayList<InParameter> inputParam = new ArrayList<>();
		ArrayList<OutParameter> outParam = new ArrayList<>();
		inputParam.add(new InParameter("av_module_item_id", DBEngineConstants.TYPE_LONG, moduleItemId));
		inputParam.add(new InParameter("av_module_code", DBEngineConstants.TYPE_INTEGER, moduleCode));
		outParam.add(new OutParameter("resultset", DBEngineConstants.TYPE_RESULTSET));
		ArrayList<HashMap<String, Object>> QuestionnaireAnswerDtoMap = dbEngine.executeProcedure(inputParam,"get_mitkc_qnr_qstn_answer", outParam);
		return QuestionnaireAnswerDtoMap;
	}

	private ArrayList<HashMap<String, Object>> getQuestionnaireAnswer(Integer questionnaireAnswerHeader) throws Exception {
		ArrayList<InParameter> inputParam = new ArrayList<>();
		ArrayList<OutParameter> outParam = new ArrayList<>();
		inputParam.add(new InParameter("av_qnr_ans_header_id", DBEngineConstants.TYPE_INTEGER, questionnaireAnswerHeader));
		outParam.add(new OutParameter("resultset", DBEngineConstants.TYPE_RESULTSET));
		ArrayList<HashMap<String, Object>> QuestionnaireAnswerDtoMap = dbEngine.executeProcedure(inputParam,"get_mitkc_qnr_answer", outParam);
		return QuestionnaireAnswerDtoMap;
	}

	@Override
	public QuestionnaireDto getQuestionnaireDetails(Integer moduleCode, String moduleItemId) throws Exception {
		return getQuestionnaireDetails(moduleCode, moduleItemId, null);
	}
			...
			for (int i = 0; i < questionnaireJsnArray.length(); i++) {
				...
				if (!explrObject.get("answerType").equals("Attachment") && !explrObject.get("acType").equals(null)) {
					...
					if (explrObject.get("answerNumber").equals(null)) {
						inParam.add(new InParameter("av_answer_number", DBEngineConstants.TYPE_INTEGER, 1)); 
					} else {
				...
				} else if (explrObject.get("answerType").equals("Attachment") && !explrObject.get("acType").equals(null)
						&& !explrObject.get("acType").equals("U")) {
					inParam.add(new InParameter("av_module_item_id", DBEngineConstants.TYPE_LONG, moduleItemId));
					inParam.add(new InParameter("av_module_item_code", DBEngineConstants.TYPE_INTEGER, moduleCode));
					inParam.add(new InParameter("av_questionnaire_id", DBEngineConstants.TYPE_INTEGER, questionnaireId));
					inParam.add(new InParameter("av_questionnaire_ans_header_id", DBEngineConstants.TYPE_INTEGER,questionnaireAnsHeaderId));
	...
	private Integer generateQuestionnaireId(Integer moduleCode, String moduleItemId) {
		...
		try {
			...
			ArrayList<HashMap<String, Object>> result = dbEngine.executeFunction("fn_mitkc_irb_questionaire_id",outParam);
			if (result != null && !result.isEmpty()) {
				HashMap<String, Object> hmResult = result.get(0);
